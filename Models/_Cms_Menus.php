...
class Cms_Menus extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'menu' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'post' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'men_menu' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'title' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
                'description' => ['type' => 'TEXT', 'null' => FALSE],
                'parent' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
                'target' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
     ...
     * @return array|false
     */
    public function get_Menu($menu): false|array
    {
        $key = $this->get_CacheKey("menu-{$menu}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $menu)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
