...
class Cms_Links extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'link' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
     ...
     * @return array|false
     */
    public function get_Link($link): false|array
    {
        $key = $this->get_CacheKey("link-{$link}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $link)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
