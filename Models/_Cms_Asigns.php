...
/**
 * Ej: $model = model('App\Modules\Cms\Models\Cms_Asigns');
 * @method where(mixed $primaryKey, string $id) : \Higgs\Database\BaseBuilder
 * @method groupStart() : \Higgs\Database\BaseBuilder
 */
class Cms_Asigns extends Model
{
    protected $table = "cms_asigns";
    protected $primaryKey = "asign";
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'asign' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'post' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'meta' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
     ...
     * @return array|false
     */
    public function get_Asign($asign): false|array
    {
        $key = $this->get_CacheKey("asign-{$asign}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $asign)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
