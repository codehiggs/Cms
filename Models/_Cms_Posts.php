...
class Cms_Posts extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'body' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
     ...
     * @return array|false
     */
    public function get_Post($post): false|array
    {
        $key = $this->get_CacheKey("post-{$post}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $post)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
