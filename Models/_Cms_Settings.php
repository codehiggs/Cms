...
/**
 * Ej: $model = model('App\Modules\Cms\Models\Cms_Settings');
...
class Cms_Settings extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                'setting' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
                'site' => ['type' => 'VARCHAR', 'constraint' => 254, 'null' => FALSE],
     ...
     * @return array|false
     */
    public function get_Setting($setting): false|array
    {
        $key = $this->get_CacheKey("setting-{$setting}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $setting)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
