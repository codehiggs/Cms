...
class Cms_Blocks extends Model
{
     ...
     * @return array|false
     */
    public function get_Block($block): false|array
    {
        $key = $this->get_CacheKey("block-{$block}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $block)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
