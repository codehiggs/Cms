...
class Cms_Components extends Model
{
    ...
    private function exec_TableRegenerate()
    {
        if (!$this->get_TableExist()) {
            ...
            $fields = [
                ...
                'widget' => ['type' => 'VARCHAR', 'constraint' => 13, 'null' => FALSE],
    ...
    public function get_List(int $limit, int $offset, string $search = ""): array|false
    {
        $result = $this
            ->groupStart()
            ->like("component", "%{$search}%")
            ->orLike("post", "%{$search}%")
            ->orLike("widget", "%{$search}%")
            ->orLike("block", "%{$search}%")
     ...
     * @return array|false
     */
    public function get_Component($component): false|array
    {
        $key = $this->get_CacheKey("component-{$component}");
        $cache = cache($key);
        if (!$this->is_CacheValid($cache)) {
            $row = $this->where($this->primaryKey, $component)->first();
            if (is_array($row)) {
                $value = $row;
            } else {
                $value = false;
