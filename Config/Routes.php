...
if (strpos($dirName, '_') === false) {
    ...
    $mdm = model("App\Modules\Cms\Models\Cms_Modules");
    $mdcxm = model("App\Modules\Cms\Models\Cms_Clients_Modules");
    $module = 'cms';
    ...
    if ($cxm == "authorized") {
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Cms::index');
                $subroutes->add('/home', 'Cms::index');
                $subroutes->add('home/(:any)', 'Cms::home/$1');
        ...
        $routes->group($module,
            ['namespace' => $namespace],
            function ($subroutes) {
                $subroutes->add('/', 'Cms::denied');
                $subroutes->add('(:any)', 'Cms::denied');
            }
        );
    }
}
